...
function buildNutritionView(p){
  ...
  const focus = p.mainGoal==='fat_loss' ? 'créer un léger déficit avec satiété élevée' :
    p.mainGoal==='muscle_gain' ? 'sécuriser un surplus mesuré et une bonne répartition protéique' :
    ['hyrox','trail','conditioning','endurance'].includes(p.mainGoal) ? 'soutenir l énergie, l hydratation et la récupération' :
    'stabiliser l alimentation et la récupération';
  return `<div class="panel panel-hero"><h3>Nutrition FAFATRAINING</h3>
    <div class="summary-grid">
      <div class="summary-card"><strong>${n.kcal}</strong><div class="small-muted">kcal repère</div></div>
    ...
    <p><strong>Objectif nutrition :</strong> ${focus}.</p>
    <p><strong>Repères coach :</strong> 3 à 4 repas stables, protéines réparties, légumes sur les repas principaux, hydratation régulière et collation utile autour des séances si besoin.</p>
    <p><strong>Restrictions connues :</strong> ${p.restrictions || 'aucune restriction déclarée'}.</p>
    ${food.length?`<p><strong>Préférences alimentaires :</strong> ${food.join(', ')}.</p>`:''}
    <p class="small-muted">Lecture simple : point de départ à ajuster selon évolution du poids, énergie, faim et assiduité.</p></div>`;
}
