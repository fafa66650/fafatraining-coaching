...
function generateShareLink(){
  ...
  if(out){
    out.insertAdjacentHTML('afterbegin', `<div class="panel" style="margin-bottom:12px"><h4>Lien adhérent</h4><p class="helper">Lien direct à copier et envoyer par SMS, WhatsApp ou mail.</p><div class="copybox">${esc(link)}</div></div>`);
  }
  if(navigator.clipboard?.writeText){
    navigator.clipboard.writeText(link).then(()=>alert('Lien adhérent copié.')).catch(()=>window.prompt('Copie ce lien adhérent :', link));
...
function printProgram(){
  ...
  if(window.jspdf && window.jspdf.jsPDF){
    ...
    (p.days||[]).forEach(day=>{
      ...
      (day.items||[]).forEach(ex=>{
        addLine(`${ex.name} — ${ex.prescription}`, 9, true);
  ...
  a.download = file.replace('.pdf','.html');
  a.click();
...
function hydrateAthleteFromLink(){
  ...
  const hash = new URLSearchParams(rawHash.replace(/&/g,'&'));
  const code = (hash.get('client') || search.get('client') || '').trim().toUpperCase();
  const payload = hash.get('payload') || search.get('payload');
  if(payload){
