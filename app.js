...
function bindCoachSmartSync(){
  ...
  const autoFill = ()=>{
    ...
    if(mainGoal && cycleGoal){
      ...
      if(cycleMap[main]) setMultiValues('cycleGoal', cycleMap[main]);
    }
    const moduleMap = [];
    if(sport==='boxing' || main==='boxing') moduleMap.push('boxing_prep');
    if(sport==='running' || main==='endurance') moduleMap.push('trail_prep');
    if(main==='hyrox' || sport==='gym') moduleMap.push('hyrox_prep');
    if(main==='fat_loss') moduleMap.push('express_fat_loss');
    if($('#medicalFlag')?.value === 'yes') moduleMap.push('return_to_play');
    if(moduleMap.length) setMultiValues('specialModule', [...new Set(moduleMap)]);
    const coachTypes = [];
    if(main==='boxing') coachTypes.push('boxing');
    if(main==='recovery' || $('#medicalFlag')?.value==='yes') coachTypes.push('health');
    if(main==='mobility') coachTypes.push('stretching');
    if(['fat_loss','conditioning','hyrox','muscle_gain','strength'].includes(main)) coachTypes.push('fitness');
    coachTypes.push('general');
    setMultiValues('coachingType', [...new Set(coachTypes)]);
  ...
  ['trainingPreference','currentSport','mainGoal','medicalFlag','availabilityWeekly','clientLevel'].forEach(id=>{
...
function buildEquipmentGrid(){
  ...
  $('#equipmentGrid').innerHTML = `<label>Bibliothèque matériel<select id="equipmentMulti" multiple size="16">${labels}</select></label>`;
  const syncEnv = ()=>{
  ...
  $('#libraryOutput').innerHTML = `
    <div class="panel">
      ...
      return `<article class="library-card">
        ...
        <p><strong>Lecture débutant :</strong> ${esc(ex.beginner_explain || 'Version simple à intégrer')}</p>
        <p><strong>Matériel :</strong> ${(ex.equipment||[]).map(eqLabel).join(' · ') || 'Poids du corps / adaptable'}</p>
      </article>`;
  }).join('') : '<div class="panel"><p>Aucun exercice trouvé avec ces filtres. Réinitialise un filtre ou élargis ta recherche.</p></div>');
}
