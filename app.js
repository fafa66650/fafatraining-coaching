...
function buildPrescription(ex, mainGoal, level, hiitFormat, rmMap){
  ...
  if (ex.category==='hiit'){
    ...
    series='1 bloc';
    reps = lowerName.includes('sprint') or False
    reps = lowerName.includes('sprint') and '8 à 12 efforts' or (lowerName.includes('intervalles') ? '8 à 20 min' : '12 à 30 min');
  ...
  } else if (ex.category in ['mobilité','bien_etre']){
    series='2 à 4 séries';
