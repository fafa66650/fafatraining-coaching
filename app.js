...
function scoreExercise(ex, ctx){
  let score = 0;
  const text = `${ex.name} ${ex.category} ${ex.subcategory} ${ex.muscles} ${(ex.tags||[]).join(' ')} ${(ex.focus||[]).join(' ')}`.toLowerCase();
  if(ex.level === ctx.level) score += 18;
  else if(ctx.level === 'advanced' && ex.level === 'intermediate') score += 10;
  else if(ctx.level === 'intermediate' && ex.level === 'beginner') score += 6;
  if(ctx.env && ex.environments?.includes(ctx.env)) score += 18;
  if(ctx.env === 'gym' && ex.environments?.includes('gym')) score += 22;
  if(ctx.env === 'bodyweight_only' && ex.equipment?.includes('bodyweight')) score += 25;
  if(ctx.equipment.length && ex.equipment?.some(e=>ctx.equipment.includes(e))) score += 16;
  if(ctx.goal && (ex.tags||[]).includes(ctx.goal)) score += 26;
  if((ctx.secondGoals||[]).some(g=>(ex.tags||[]).includes(g))) score += 10;
  if((ctx.keywords||[]).some(k=>text.includes(k))) score += 18;
  if((ctx.focusTargets||[]).some(k=>text.includes(k.replace('_',' ')))) score += 10;
  if(ctx.loadedPriority && ex.equipment && ex.equipment.some(e=>['barbell','dumbbell','machine','cable','trap_bar','landmine','rack','bench'].includes(e))) score += 20;
  if(ctx.bodyweightPriority && ex.equipment?.includes('bodyweight')) score += 15;
  if(ctx.avoidBurpee && /burpee/i.test(text)) score -= 50;
  if(ctx.medical.includes('hypertension') && /(all out|max|sprint)/i.test(text)) score -= 40;
  if(ctx.injuries.includes('genou') && /(jump|bond|saut)/i.test(text)) score -= 35;
  if(ctx.injuries.includes('dos') && /(good morning|deadlift lourd|rotation explosive)/i.test(text)) score -= 35;
...
function pickExercises(ctx, count=4){
  const pool = EXERCISES.filter(ex => {
    ...
    return true;
  }).map(ex => ({ex, score: scoreExercise(ex, ctx)})).filter(x=>x.score > 5).sort((a,b)=>b.score-a.score);
  const chosen = [];
...
function prescriptionFor(goal, title, block, ex, form){
  const eq = ex.equipment || [];
  const loaded = eq.some(e=>['barbell','dumbbell','machine','cable','kettlebell','trap_bar','landmine','sandbag'].includes(e));
  const isMob = /mobil|stretch|respiration|flow|souplesse/i.test(`${ex.name} ${ex.category} ${ex.subcategory}`);
  if(block === 'warmup') return {series:'2 séries', work:'20 à 30 sec ou 6 à 8 reps', rest:'15 à 20 sec', load:'Montée progressive'};
  if(block === 'cooldown') return {series:'1 à 2 passages', work:isMob ? '30 à 45 sec' : '5 à 8 reps lentes', rest:'10 à 20 sec', load:'Respiration calme'};
  if(goal === 'strength') return {series:'4 à 5 séries', work: loaded ? '4 à 6 reps' : '6 à 8 reps', rest:'90 à 150 sec', load:loadGuidance(form, ex)};
  if(goal === 'muscle_gain' || goal === 'recomposition') return {series:'3 à 4 séries', work: loaded ? '6 à 12 reps' : '8 à 15 reps', rest:'60 à 90 sec', load:loadGuidance(form, ex)};
  if(goal === 'boxing') return {series:'3 à 5 rounds', work:'45 à 90 sec', rest:'30 à 45 sec', load:eq.includes('heavy_bag') ? 'Impact contrôlé' : 'Vitesse / précision'};
  if(goal === 'hyrox') return {series:'3 à 4 séries', work: loaded ? '8 à 12 reps ou 200 à 500 m' : '30 à 60 sec', rest:'45 à 75 sec', load:loadGuidance(form, ex)};
  if(goal === 'trail' || goal === 'endurance') return {series:'3 à 4 séries', work: loaded ? '8 à 12 reps' : '30 à 60 sec', rest:'45 à 60 sec', load:loaded ? loadGuidance(form, ex) : 'Allure contrôlée'};
  if(goal === 'mobility' || goal === 'health' || goal === 'return_to_play') return {series:'2 à 3 séries', work:isMob ? '30 à 45 sec' : '8 à 12 reps', rest:'20 à 40 sec', load:loaded ? 'Charge légère à modérée' : 'Amplitude confortable'};
  return {series:'3 à 4 séries', work: loaded ? '6 à 10 reps' : '8 à 12 reps', rest:'40 à 75 sec', load:loadGuidance(form, ex)};
}
...
function quickPrescription(style, zone, env){
  const bodyOnly = env === 'bodyweight_only';
  const maps = {
    hiit:{warmup:{series:'2 séries', work:'20 sec', rest:'15 sec'}, main:{series:'4 à 6 séries', work:'30 à 40 sec', rest:'20 à 30 sec'}, cooldown:{series:'1 à 2 passages', work:'30 sec', rest:'15 sec'}},
    emom:{warmup:{series:'2 séries', work:'20 sec', rest:'15 sec'}, main:{series:'8 à 12 minutes', work:'1 exercice chaque minute', rest:'reste de la minute'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    amrap:{warmup:{series:'2 séries', work:'20 sec', rest:'15 sec'}, main:{series:'1 bloc', work:'8 à 15 minutes', rest:'repos libre si besoin'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    strength:{warmup:{series:'2 séries', work:'6 reps', rest:'20 sec'}, main:{series:'4 séries', work: bodyOnly ? '8 à 12 reps' : '4 à 8 reps', rest:'90 sec'}, cooldown:{series:'1 passage', work:'30 sec', rest:'15 sec'}},
    boxing:{warmup:{series:'2 séries', work:'30 sec', rest:'15 sec'}, main:{series:'4 rounds', work:'45 à 90 sec', rest:'30 sec'}, cooldown:{series:'1 passage', work:'2 min', rest:'—'}},
    mobility:{warmup:{series:'1 passage', work:'3 min', rest:'—'}, main:{series:'2 à 3 séries', work:'30 à 45 sec', rest:'15 à 20 sec'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    recovery:{warmup:{series:'1 passage', work:'2 min', rest:'—'}, main:{series:'2 séries', work:'30 sec', rest:'15 sec'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    zone2:{warmup:{series:'1 passage', work:'4 min', rest:'—'}, main:{series:'1 bloc', work:'20 à 40 min allure facile', rest:'—'}, cooldown:{series:'1 passage', work:'4 min', rest:'—'}},
    trail:{warmup:{series:'1 passage', work:'4 min', rest:'—'}, main:{series:'3 à 4 blocs', work:'2 à 5 min', rest:'60 sec'}, cooldown:{series:'1 passage', work:'4 min', rest:'—'}},
    hyrox:{warmup:{series:'2 séries', work:'20 sec', rest:'15 sec'}, main:{series:'3 à 4 blocs', work:'40 à 60 sec', rest:'30 sec'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    core:{warmup:{series:'1 passage', work:'2 min', rest:'—'}, main:{series:'3 séries', work:'20 à 40 sec', rest:'20 sec'}, cooldown:{series:'1 passage', work:'2 min', rest:'—'}},
    health:{warmup:{series:'1 passage', work:'2 min', rest:'—'}, main:{series:'2 à 3 séries', work:'8 à 12 reps', rest:'30 à 45 sec'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    conditioning:{warmup:{series:'2 séries', work:'20 sec', rest:'15 sec'}, main:{series:'3 à 5 séries', work:'30 à 45 sec', rest:'30 sec'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}},
    circuit:{warmup:{series:'2 séries', work:'20 sec', rest:'15 sec'}, main:{series:'3 à 4 tours', work:'8 à 12 reps ou 30 sec', rest:'20 à 30 sec entre exercices'}, cooldown:{series:'1 passage', work:'3 min', rest:'—'}}
  };
...
function buildQuickSession(){
  ...
  const ctx = {goal: style === 'zone2' ? 'trail' : style, secondGoals:[], level: audience==='athletes' ? 'advanced' : audience==='adults' ? 'intermediate' : 'beginner', env, equipment: PRESETS[env] || ['bodyweight'], keywords: style==='mobility' || style==='recovery' ? ['mobil','stretch','respiration'] : style==='boxing' ? ['boxe','shadow','bag','pads'] : style==='strength' ? ['squat','tirage','développ','hinge'] : style==='hyrox' ? ['carry','rower','bike','sled'] : style==='trail' || style==='zone2' ? ['course','marche','endurance'] : ['cardio','full body'], focusTargets:[], loadedPriority:['gym','crossfit_box'].includes(env) && style==='strength', bodyweightPriority:env==='bodyweight_only', medical:[], injuries:[], exclude:[]};
  const warmup = pickExercises({...ctx, goal:'mobility', keywords:['mobil','activation','respiration','rotation'], bodyweightPriority:true}, 2);
  const main = pickExercises({...ctx, avoidBurpee: style!=='conditioning' && style!=='hiit'}, style==='zone2' ? 1 : 4);
  const cooldown = pickExercises({...ctx, goal:'mobility', keywords:['stretch','mobil','respiration','marche'], bodyweightPriority:true, exclude:[...warmup,...main].map(baseName)}, 1);
  const blocks = [
    {title:'Échauffement', items:warmup, prescription:quickPrescription(style,'warmup',env)},
    {title: style==='zone2' ? 'Bloc principal cardio' : 'Corps de séance', items:main, prescription:quickPrescription(style,'main',env)},
    {title:'Retour au calme', items:cooldown, prescription:quickPrescription(style,'cooldown',env)}
  ];
  $('#quickOutput').innerHTML = `<div class="quick-layout"><div class="stack">${blocks.map(block=>`<article class="panel quick-block"><h3>${esc(block.title)}</h3>${block.items.map(ex=>`<div class="program-ex-card"><h4>${esc(ex.name)}</h4><div class="small-muted">${esc(ex.muscles || '')}</div><div class="meta-line"><span>${esc(block.prescription.series)}</span><span>${esc(block.prescription.work)}</span><span>Repos ${esc(block.prescription.rest)}</span>${!((ex.equipment||[]).includes('bodyweight') && !(ex.equipment||[]).some(e=>['barbell','dumbbell','machine','cable','kettlebell','trap_bar','landmine','sandbag'].includes(e))) ? `<span>${esc(loadGuidance({level:ctx.level}, ex))}</span>` : ''}</div><div class="session-line"><strong>Consigne :</strong> ${esc(ex.cue)}</div><div class="session-line"><strong>Version plus facile :</strong> ${esc(ex.easy || 'Réduire amplitude ou vitesse')}</div><div class="session-line"><strong>Version plus difficile :</strong> ${esc(ex.hard || 'Augmenter légèrement l’intensité')}</div></div>`).join('')}</article>`).join('')}</div><aside class="panel"><h3>Logique de séance</h3><div class="session-line"><strong>Public :</strong> ${esc({kids:'Enfants',teens:'Ados',adults:'Adultes',seniors:'Seniors',athletes:'Sportifs avancés'}[audience])}</div><div class="session-line"><strong>Style :</strong> ${esc(formatLabel(style))}</div><div class="session-line"><strong>Lieu :</strong> ${esc(envLabel(env))}</div><div class="session-line"><strong>Durée :</strong> ${esc(String(duration))} min</div><div class="session-line"><strong>Lecture pratique :</strong> On termine les séries ou le temps indiqués pour un exercice, puis on prend le repos prévu avant de passer au suivant. En EMOM, le repos correspond au temps restant dans la minute.</div><div class="session-line"><strong>Charge :</strong> En poids du corps, aucune charge externe n’est demandée. En salle, on choisit une charge qui laisse une exécution propre et 1 à 3 répétitions en réserve selon le niveau.</div><div class="panel subtle"><h4>Glossaire</h4><div class="session-line">HIIT : Intervalles haute intensité</div><div class="session-line">EMOM : Chaque minute sur la minute</div><div class="session-line">AMRAP : Maximum de tours</div><div class="session-line">Zone 2 : Endurance fondamentale</div></div></aside></div>`;
}
