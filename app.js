...
/* === V30 direction artistique + moteur salle premium === */
(function(){
  const FR_MAP={
    strength:'Force / musculation', muscle_gain:'Prise de muscle', recomposition:'Recomposition corporelle',
    conditioning:'Condition physique', boxing:'Boxe', hyrox:'Hyrox (course + ateliers fonctionnels)',
    trail:'Trail / course nature', mobility:'Mobilité / stretching', health:'Santé / remise en forme',
    return_to_play:'Retour de blessure', endurance:'Endurance', recovery:'Récupération active',
    hiit:'HIIT (intervalles haute intensité)', emom:'EMOM (chaque minute sur la minute)',
    amrap:'AMRAP (maximum de tours)', interval:'Intervalles (effort / récupération)',
    zone2:'Zone 2 (endurance fondamentale)', core:'Gainage / ceinture abdominale',
    classic:'Classique', circuit:'Circuit training', boxing_rounds:'Rounds boxe (rounds chronométrés)',
    general:'Général (polyvalent)', fitness:'Fitness (forme générale)', nutrition:'Nutrition (alimentation)',
    reath:'Réathlétisation', performance:'Performance générale', wellbeing:'Bien-être',
    athletic:'Préparation athlétique', weightlifting:'Haltérophilie', hybrid:'Hybride',
    messaging:'Messagerie / feedback', autonome:'Autonome guidé', home:'Maison', gym:'Salle de musculation',
    outdoor:'Extérieur', video:'Visio', mixed:'Mixte', remote:'Suivi à distance',
    in_person_gym:'Présentiel salle', in_person_outdoor:'Présentiel extérieur', program_only:'Programme seul',
    bodyweight_only:'Poids du corps uniquement', crossfit_box:'Salle CrossFit / Hyrox', boxing_gym:'Salle de boxe'
  };
  const GOAL_EXPLAIN={
    fat_loss:'priorité à la dépense utile, à la régularité et au renforcement global',
    muscle_gain:'priorité aux mouvements de musculation, à la charge progressive et à la récupération',
    strength:'priorité aux exercices de base, à la technique et à la montée en charge',
    recomposition:'priorité au renforcement musculaire et au volume maîtrisé',
    conditioning:'priorité au moteur cardio-musculaire et aux formats dynamiques',
    boxing:'priorité à la boxe, au jeu de jambes, au gainage et au cardio spécifique',
    hyrox:'priorité à la course, aux carries, au sled, au rameur/ski et au gainage',
    trail:'priorité à l’endurance, aux jambes, aux mollets et à la stabilité',
    mobility:'priorité à la mobilité, au contrôle et à l’amplitude',
    health:'priorité à la sécurité, à la posture et à la remise en forme',
    return_to_play:'priorité à la reprise progressive et aux variantes stables'
  };

  window.calcBMI = function(weightKg,heightCm,meta={}){
    const h = Number(heightCm)/100;
    ...
    let risk='formule officielle : poids (kg) / taille² (m). L’IMC seul ne décrit pas toute la composition corporelle.';
    if((meta.activityLevel==='very_active' || meta.activityLevel==='active') && meta.level==='advanced' && bmi >= 27){
      label='IMC élevé à interpréter avec prudence';
      risk='formule officielle : poids (kg) / taille² (m). Profil potentiellement musclé : compléter avec photos, mensurations et tour de taille.';
    }
    const waistCm = Number(meta.waistCm||0);
    const sex = meta.sex==='female' ? 'female' : 'male';
    if(waistCm){
      const limit = sex==='female' ? 88 : 102;
      risk += waistCm>=limit ? ` Tour de taille élevé (seuil usuel ${limit} cm).` : ` Tour de taille rassurant sous le seuil usuel ${limit} cm.`;
    }
    return {value:bmi.toFixed(1), label, risk};
  };

  function explainRest(style, block){
    if(style==='emom') return 'Chaque nouvelle minute lance le travail prévu. Le repos correspond au temps restant dans la minute.';
    if(style==='amrap') return 'Réalise un maximum de tours propres dans le temps donné. Repos court seulement si la technique baisse.';
    if(style==='interval') return 'Travaille pendant le temps effort indiqué, puis récupère pendant le temps repos avant de repartir.';
    if(style==='zone2') return 'Reste à allure confortable : respiration contrôlée, phrase courte possible. Le repos est optionnel et très léger.';
    if(block==='warmup') return 'Enchaîne calmement pour préparer les articulations. Repos court entre exercices.';
    if(block==='finisher') return 'Retour au calme : respiration, relâchement et qualité du mouvement avant tout.';
    return 'Repos à prendre après chaque série ou chaque tour selon le format affiché à côté de l’exercice.';
  }

  window.prescriptionForQuick = function(style, block, dur){
    const d=Number(dur||45);
    if(style==='mobility' || style==='recovery') return {
      series:block==='main'?'2 à 3 tours':'1 à 2 passages',
      reps:'30 à 45 sec par mouvement',
      rest:'15 à 25 sec',
      note:explainRest(style,block)
    };
    if(style==='emom') return {series:d<=30?'10 à 12 min':'12 à 16 min', reps:'1 exercice / minute', rest:'reste de la minute', note:explainRest(style,block)};
    if(style==='amrap') return {series:d<=30?'8 à 12 min':'12 à 18 min', reps:'maximum de tours propres', rest:'micro-repos si besoin', note:explainRest(style,block)};
    if(style==='interval') return {series:d<=30?'6 à 8 blocs':'8 à 10 blocs', reps:'30 à 40 sec effort', rest:'20 à 30 sec', note:explainRest(style,block)};
    if(style==='zone2') return {series:'1 bloc continu', reps:d<=30?'18 à 25 min':'25 à 40 min', rest:'léger si besoin', note:explainRest(style,block)};
    if(style==='strength') return {series:'3 à 5 séries', reps:'4 à 8 reps', rest:'75 à 120 sec', note:explainRest(style,block)};
    return {series:'3 à 4 séries', reps:'8 à 15 reps / 30 à 45 sec', rest:'30 à 60 sec', note:explainRest(style,block)};
  };

  function overlayPosition(host, menu){
    const rect=host.getBoundingClientRect();
    const desiredWidth=Math.min(Math.max(rect.width, 320), window.innerWidth-24);
    menu.style.width=desiredWidth+'px';
    menu.style.left=Math.max(12, Math.min(rect.left, window.innerWidth-desiredWidth-12))+'px';
    menu.style.top=Math.min(rect.bottom+8, window.innerHeight-340)+'px';
  }

  function buildReliableMultiSelect(select){
    if(!select || select.dataset.v30Enhanced==='1') return;
    const legacyWrapper = select.parentElement && select.parentElement.classList && select.parentElement.classList.contains('multi-select') ? select.parentElement : null;
    select.dataset.v30Enhanced='1';
    if(legacyWrapper) legacyWrapper.after(select);
    if(legacyWrapper) legacyWrapper.remove();
    const existing=select.parentElement && !legacyWrapper ? select.parentElement.querySelector('.multi-select') : null;
    if(existing) existing.remove();
    select.style.display='none';
    const host=document.createElement('div');
    host.className='multi-select';
    host.innerHTML='<button type="button" class="multi-select-trigger" aria-expanded="false"><span class="multi-select-value">Aucune sélection</span><span class="multi-select-arrow">▾</span></button><div class="multi-select-tags"></div><div class="multi-select-menu" hidden><div class="multi-select-search-wrap"><input type="text" class="multi-select-search" placeholder="Rechercher..."></div><div class="multi-select-options"></div></div>';
    select.after(host); host.appendChild(select);
    const trigger=host.querySelector('.multi-select-trigger');
    const value=host.querySelector('.multi-select-value');
    const tags=host.querySelector('.multi-select-tags');
    const menu=host.querySelector('.multi-select-menu');
    const search=host.querySelector('.multi-select-search');
    const optionsWrap=host.querySelector('.multi-select-options');
    const closeAll=()=>document.querySelectorAll('.multi-select.open').forEach(h=>{h.classList.remove('open');const m=h.querySelector('.multi-select-menu'); if(m) m.hidden=true; const t=h.querySelector('.multi-select-trigger'); if(t) t.setAttribute('aria-expanded','false');});
    const refresh=()=>{
      const chosen=Array.from(select.selectedOptions);
      value.textContent=chosen.length ? `${chosen.length} sélection${chosen.length>1?'s':''}` : 'Aucune sélection';
      tags.innerHTML=chosen.length ? chosen.map(o=>`<span class="tag">${o.textContent}</span>`).join('') : '';
      paint();
    };
    const paint=()=>{
    ...
    trigger.addEventListener('click',e=>{
      ...
      closeAll();
      if(opening){
        host.classList.add('open'); menu.hidden=false; trigger.setAttribute('aria-expanded','true'); overlayPosition(host,menu); setTimeout(()=>search.focus(),0);
      }
    });
    search.addEventListener('input',paint);
    window.addEventListener('resize',()=>{ if(!menu.hidden) overlayPosition(host,menu); });
    document.addEventListener('scroll',()=>{ if(!menu.hidden) overlayPosition(host,menu); }, true);
  ...
  function relabelUI(){
    ...
    if(quickStyle){
      const opts=[['circuit','Circuit training'],['hiit','HIIT (intervalles haute intensité)'],['emom','EMOM (chaque minute sur la minute)'],['amrap','AMRAP (maximum de tours)'],['interval','Intervalles (effort / récupération)'],['zone2','Zone 2 (endurance fondamentale)'],['strength','Force / musculation'],['boxing','Boxe'],['conditioning','Condition physique'],['hyrox','Hyrox (course + ateliers fonctionnels)'],['trail','Trail / course nature'],['mobility','Mobilité / stretching'],['core','Gainage / ceinture abdominale'],['health','Santé / remise en forme'],['recovery','Récupération active']];
      quickStyle.innerHTML=opts.map(([v,l])=>`<option value="${v}">${l}</option>`).join('');
    }
    document.querySelectorAll('option').forEach(opt=>{ const k=(opt.value||'').toLowerCase(); if(FR_MAP[k]) opt.textContent=FR_MAP[k]; });
  }

  function refineProfileLabels(){
    const cs=document.querySelector('label[for="currentSport"]');
    const field=document.getElementById('currentSport');
    if(field && field.parentElement && field.parentElement.tagName==='LABEL') field.parentElement.firstChild.textContent='Discipline actuelle (optionnel)';
  }

  function normalizeFormSizing(){
    document.querySelectorAll('.form-grid label').forEach(l=>{ l.style.minHeight='auto'; });
    document.querySelectorAll('.form-grid select[multiple]').forEach(buildReliableMultiSelect);
  }

  function dedupe(arr){ return [...new Set((arr||[]).filter(Boolean))]; }

  function goalKeys(goal, env, title=''){
    const lower=(title||'').toLowerCase();
    if((env==='gym' || env==='crossfit_box' || env==='boxing_gym') && ['muscle_gain','strength','recomposition'].includes(goal)){
      if(/pouss|push|haut 1/.test(lower)) return ['press','bench','chest','shoulders','triceps'];
      if(/tirage|pull|haut 2/.test(lower)) return ['row','pull','back','biceps','rear'];
      if(/jambe|squat|lower 1/.test(lower)) return ['squat','quads','glutes','leg press','split squat'];
      if(/charni|hinge|ischio|lower 2/.test(lower)) return ['hinge','hamstrings','glutes','rdl','hip thrust'];
      return ['bench','row','squat','hinge','press','pull'];
    }
    if(goal==='boxing') return ['boxing','rotation','core','single_leg','conditioning'];
    if(goal==='hyrox') return ['carry','sled','row','ski','burpee','lunge'];
    if(goal==='trail' || goal==='endurance') return ['single_leg','calves','hamstrings','cardio','stability'];
    if(goal==='mobility' || goal==='health' || goal==='return_to_play') return ['mobility','stability','breathing','rotation','core'];
    if(goal==='fat_loss' || goal==='conditioning') return ['squat','push','pull','cardio','core'];
    return ['push','pull','squat','hinge','core'];
  }

  function pickExercises(form, title, count){
    const level=form.level||'intermediate';
    const env=form.env||'home';
    const equipment=form.equipment||[];
    const style=((env==='gym'||env==='crossfit_box') && ['strength','muscle_gain','recomposition'].includes(form.mainGoal)) ? form.mainGoal : (form.mainGoal||'conditioning');
    const keys=goalKeys(form.mainGoal, env, title);
    const chosen=chooseDiverseExercises({level, env, equipment, style, goals:keys, count, excludeBases:[]}) || [];
    return chosen;
  }

  function titleSet(form){
    const freq=Number(form.freq||3);
    if((form.env==='gym'||form.env==='crossfit_box'||form.env==='boxing_gym') && ['strength','muscle_gain','recomposition'].includes(form.mainGoal)){
      const map={2:['Jour 1 · Haut du corps','Jour 2 · Bas du corps'],3:['Jour 1 · Poussée','Jour 2 · Tirage','Jour 3 · Jambes'],4:['Jour 1 · Poussée','Jour 2 · Tirage','Jour 3 · Jambes dominant squat','Jour 4 · Jambes dominant charnière'],5:['Jour 1 · Poussée','Jour 2 · Tirage','Jour 3 · Jambes dominant squat','Jour 4 · Haut du corps volume','Jour 5 · Jambes dominant charnière'],6:['Jour 1 · Poussée','Jour 2 · Tirage','Jour 3 · Jambes dominant squat','Jour 4 · Haut du corps volume','Jour 5 · Jambes dominant charnière','Jour 6 · Bras / gainage / finition']};
      return map[freq] || map[4];
    }
    if(form.mainGoal==='boxing') return ['Jour 1 · Technique boxe','Jour 2 · Condition physique boxe','Jour 3 · Appuis / gainage','Jour 4 · Rounds / moteur'].slice(0,freq);
    if(form.mainGoal==='mobility') return ['Jour 1 · Mobilité globale','Jour 2 · Hanches / chevilles','Jour 3 · Haut du corps / thoracique','Jour 4 · Stabilité / respiration'].slice(0,freq);
    return buildDayTitles(form.freq, form.mainGoal, (form.cycleGoals||[])[0] || '');
  }

  window.buildProgramDays = function(form){
    const titles=titleSet(form);
    return titles.map((title, di)=>{
      const picks=pickExercises(form,title, di<2?5:4);
      const items=picks.map((ex,idx)=>{
        const base=baseName(ex);
        const p=formatPrescription(form.mainGoal, title, idx);
        const isGym=['muscle_gain','strength','recomposition'].includes(form.mainGoal) && (form.env==='gym'||form.env==='crossfit_box'||form.env==='boxing_gym');
        if(isGym){
          p.series = idx<2 ? '4 séries' : '3 à 4 séries';
          p.reps = form.mainGoal==='strength' ? (idx<2?'4 à 6 reps':'6 à 8 reps') : (idx<2?'6 à 10 reps':'10 à 15 reps');
          p.rest = idx<2 ? '90 à 150 sec' : '60 à 90 sec';
        }
        return {
          ...ex,
          name: ex.name,
          category: ex.category || labelForGoal(form.mainGoal),
          muscles: ex.muscles || goalKeys(form.mainGoal, form.env, title).slice(0,2).join(', '),
          prescription:p,
          load: isGym ? loadGuidance(form.mainGoal, form.env, idx) : (ex.load||loadGuidance(form.mainGoal, form.env, idx)),
          cue: ex.cue || 'Exécution propre, respiration calée et amplitude maîtrisée.',
          substitute: ex.substitute || 'Version plus stable ou plus légère si nécessaire.',
          progression: ex.progression || 'Augmenter un peu la charge, le temps sous tension ou la difficulté technique.',
          coachNote: ex.coachNote || 'Valider la qualité du mouvement avant d’augmenter la difficulté.',
          base
        };
      });
      return {title, patternSummary: GOAL_EXPLAIN[form.mainGoal] || 'répartition équilibrée', items};
    });
  };

  const oldBuildProgram=window.buildProgram;
  window.buildProgram = function(){
    if(typeof oldBuildProgram==='function') return oldBuildProgram();
  };

  const originalQuickBuild=window.quickBuild;
  window.quickBuild = function(){
    const style=document.getElementById('quickStyle').value;
    const dur=Number(document.getElementById('quickDuration').value||45);
    const env=document.getElementById('quickEnv').value;
    const audience=document.getElementById('quickPublic').value;
    const audienceHints={kids:'ludique, simple et sécurisé',teens:'dynamique, progressif et engageant',adults:'équilibré, utile et adaptable',seniors:'contrôlé, stable et sécurisant',athletes:'plus dense, plus spécifique et exigeant'};
    const styleMap={amrap:'conditioning',emom:'conditioning',interval:'conditioning',zone2:'trail'};
    const realStyle=styleMap[style]||style;
    const warmup=chooseDiverseExercises({level:audience==='athletes'?'advanced':'intermediate',env,equipment:PRESETS[env]||[],style:realStyle,goals:realStyle==='mobility'?['mobility','stability','rotation']:['mobility','core','cardio'],count:3,excludeBases:[]});
    const main=chooseDiverseExercises({level:audience==='athletes'?'advanced':'intermediate',env,equipment:PRESETS[env]||[],style:realStyle,goals:goalKeys(realStyle,env,''),count:5,excludeBases:warmup.map(ex=>baseName(ex))});
    const finisher=chooseDiverseExercises({level:audience==='athletes'?'advanced':'intermediate',env,equipment:PRESETS[env]||[],style:realStyle==='strength'?'mobility':realStyle,goals:realStyle==='mobility'||realStyle==='recovery'?['breathing','mobility','stability']:['core','mobility','cardio'],count:2,excludeBases:warmup.concat(main).map(ex=>baseName(ex))});
    const sections=[['Échauffement',warmup,'warmup'],['Bloc principal',main,'main'],['Retour au calme',finisher,'finisher']];
    document.getElementById('quickOutput').innerHTML=`<div class="quick-layout"><div class="panel panel-hero">${sections.map(([title,list,key])=>{const pres=prescriptionForQuick(style,key,dur);return `<div class="quick-block"><h4>${title}</h4><div class="quick-list">${list.map(ex=>`<div class="session-item"><strong>${esc(ex.name)}</strong><div class="small-muted">${esc(ex.category||'')} · ${esc(ex.muscles||'')}</div><div class="quick-prescription"><span>${esc(pres.series)}</span><span>${esc(pres.reps)}</span><span>Repos ${esc(pres.rest)}</span></div><div class="session-line"><strong>Consigne :</strong> ${esc(ex.cue||'Exécution propre et respiration maîtrisée.')}</div><div class="session-line"><strong>Version plus facile :</strong> ${esc(ex.substitute||'Réduire l’amplitude ou choisir une variante plus stable.')}</div><div class="session-line"><strong>Version plus difficile :</strong> ${esc(ex.progression||'Ajouter un peu d’amplitude, de vitesse ou de charge.')}</div></div>`).join('')}</div></div>`;}).join('')}</div><div class="quick-rail"><div class="panel"><h3>Logique de séance</h3><div class="small-muted">Public : ${audienceHints[audience]||'adaptable'}.</div><div class="small-muted">Style : ${esc(FR_MAP[style]||style)}.</div><div class="small-muted">Temps de repos : ${esc(explainRest(style,'main'))}</div><div class="small-muted">Lecture : on termine d’abord toutes les séries d’un exercice si rien d’autre n’est précisé.</div></div><div class="panel"><h3>Glossaire</h3><div class="nutrition-list"><div><strong>HIIT :</strong> intervalles haute intensité</div><div><strong>EMOM :</strong> chaque minute sur la minute</div><div><strong>AMRAP :</strong> maximum de tours</div><div><strong>Zone 2 :</strong> endurance fondamentale</div></div></div></div></div>`;
  };

  const oldRenderHome=window.renderHome;
  window.renderHome=function(){
    if(oldRenderHome) oldRenderHome();
    const hs=document.querySelector('.home-showcase'); if(hs) hs.remove();
    const modules=document.querySelector('#home .feature-grid');
    if(modules) modules.innerHTML='<div class="feature-card"><strong>Coach Pro</strong><span>profil, objectifs, matériel, programme</span></div><div class="feature-card"><strong>Adhérent</strong><span>code, accès, portail, historique</span></div><div class="feature-card"><strong>Nutrition</strong><span>lecture claire + exemples de repas</span></div><div class="feature-card"><strong>Business</strong><span>statut, montant, échéance et accès</span></div>';
  };

  function bindUniformActions(){
    document.querySelectorAll('.step-panel .panel-hero').forEach(p=>p.style.overflow='visible');
    relabelUI();
    normalizeFormSizing();
    refineProfileLabels();
  }
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    bindUniformActions();
    setTimeout(bindUniformActions, 250);
  });
})();

/* ===== V30.1 final usability / uniform patch ===== */
(function(){
  const $ = s => document.querySelector(s);
  const $$ = s => Array.from(document.querySelectorAll(s));
  ...
  const MULTI_CONFIG = {
    secondGoals:{required:false,help:'Choisis 0 à 3 axes complémentaires.'},
    practicePrefs:{required:false,help:'Lieux ou formats appréciés par le client.'},
    coachingTypes:{required:true,help:'Le ou les pôles de coaching réellement utiles.'},
    supports:{required:false,help:'Façon d’accompagner le client.'},
    cycleGoals:{required:false,help:'Orientation dominante du cycle en cours.'},
    focusTargets:{required:false,help:'Zones à renforcer en priorité.'},
    fafaModules:{required:false,help:'Modules spécifiques FAFATRAINING à activer.'},
    medicalKnown:{required:false,help:'Uniquement ce qui influence la programmation.'},
    injuryKnown:{required:false,help:'Douleurs, zones fragiles, retour de blessure.'},
    foodKnown:{required:false,help:'Préférences, allergies, restrictions ou aversions.'},
    equipmentSelect:{required:false,help:'Matériel réellement disponible.'},
    effortFormats:{required:false,help:'Formats d’effort appréciés ou adaptés.'}
  };

  const LABEL_MAP = {
    clientName:['Nom / prénom','obligatoire','Nom du client ou de l’adhérent.'],
    clientCode:['Code adhérent','obligatoire','Code court unique pour retrouver le programme.'],
    clientEmail:['E-mail','optionnel','Utile pour le suivi, sans bloquer la génération.'],
    clientAge:['Âge','obligatoire','Permet d’ajuster le public, la charge et la nutrition.'],
    clientSex:['Sexe','obligatoire','Utile pour les calculs énergétiques de base.'],
    clientLevel:['Niveau sportif','obligatoire','Expérience réelle en entraînement structuré.'],
    clientHeight:['Taille (cm)','obligatoire','Nécessaire pour l’IMC officiel.'],
    clientWeight:['Poids (kg)','obligatoire','Nécessaire pour l’IMC officiel et les repères nutrition.'],
    currentSport:['Pratique actuelle','optionnel','Sport déjà pratiqué en ce moment.'],
    availabilityWeekly:['Disponibilité hebdomadaire','obligatoire','Nombre de jours réalistes par semaine.'],
    activityLevel:['Niveau d’activité quotidien','obligatoire','Volume global hors entraînement : sédentaire, modéré, actif…'],
    lifeContext:['Rythme de vie','optionnel','Contexte personnel ou professionnel qui influence la récupération.'],
    waistCm:['Tour de taille (cm)','optionnel','Mesure utile pour compléter la lecture santé.'],
    mainGoal:['Objectif principal','obligatoire','Le cap numéro 1 du programme.'],
    environmentSelect:["Contexte d’entraînement",'obligatoire','Lieu ou environnement principal d’entraînement.'],
    clientFreq:['Fréquence / semaine','obligatoire','Nombre de séances structurées prévues.'],
    clientDuration:['Durée / séance','obligatoire','Durée cible d’une séance.'],
    cycleWeeks:['Durée du cycle (semaines)','obligatoire','Longueur du cycle avant réajustement.'],
    sleepHours:['Sommeil moyen','optionnel','Repère utile pour doser la charge.'],
    stressLevel:['Niveau de stress','optionnel','Aide à moduler volume et récupération.'],
    bizStatus:['Statut abonnement','obligatoire','Active ou non l’accès adhérent.'],
    bizAmount:['Montant payé (€)','optionnel','Repère business, sans effet sur la programmation.'],
    quickPublic:['Public','obligatoire','Type de public principal.'],
    quickDuration:['Durée','obligatoire','Temps réel disponible pour la séance.'],
    quickStyle:['Style de séance','obligatoire','Format dominant de la séance rapide.'],
    quickEnv:['Lieu','obligatoire','Lieu ou matériel dominant pour la séance rapide.']
  };
  ...
  const TERM_MAP = {
    ...
    zone2:'Zone 2 (endurance fondamentale)',
    interval:'Intervalles (effort / récupération)',
  ...
  function agePublicLabel(age){
    age=Number(age||0);
    if(age && age<11) return 'Enfant (6-10 ans)';
    if(age && age<18) return 'Adolescent (11-17 ans)';
    if(age>=60) return 'Senior (60 ans et +)';
    return 'Adulte (18-59 ans)';
  }

  function humanLabel(el,id){
    if(LABEL_MAP[id]) return LABEL_MAP[id];
    const txt=(el.closest('label')?.childNodes[0]?.textContent||id||'').trim();
    return [txt,'optionnel',''];
  }

  function decorateLabelFor(control){
    const label=control.closest('label'); if(!label) return;
    const [title,kind,help]=humanLabel(control,control.id);
    ...
    line.innerHTML=`<span>${title}</span><span class="field-badge ${kind==='obligatoire'?'required':'optional'}">${kind}</span>`;
    label.insertBefore(line,label.firstChild);
    if(help){ const p=document.createElement('div'); p.className='help-text'; p.textContent=help; line.after(p); }
  }

  function rebuildInlineMulti(select){
    if(!select) return;
    const parent=select.closest('.multi-inline, .multi-select');
    if(parent && parent.classList.contains('multi-inline')) return;
    const label=select.closest('label'); if(!label) return;
    if(parent && parent.classList.contains('multi-select')){ parent.before(select); parent.remove(); }
    select.style.display='none';
    const host=document.createElement('div');
    host.className='multi-inline';
    host.innerHTML='<button type="button" class="multi-inline-trigger" aria-expanded="false"><span class="multi-inline-value">Aucune sélection</span><span>▾</span></button><div class="multi-inline-tags"></div><div class="multi-inline-menu"><input type="text" class="multi-inline-search" placeholder="Rechercher..."><div class="multi-inline-options"></div></div>';
    select.after(host); host.appendChild(select);
    const trigger=host.querySelector('.multi-inline-trigger');
    const value=host.querySelector('.multi-inline-value');
    const tags=host.querySelector('.multi-inline-tags');
    const menu=host.querySelector('.multi-inline-menu');
    const search=host.querySelector('.multi-inline-search');
    const options=host.querySelector('.multi-inline-options');
    const cfg=MULTI_CONFIG[select.id]||{};
    const update=()=>{
      const chosen=Array.from(select.selectedOptions);
      value.textContent=chosen.length?`${chosen.length} sélection${chosen.length>1?'s':''}`:'Aucune sélection';
      tags.innerHTML=chosen.map(o=>`<span class="tag">${o.textContent}</span>`).join('');
      render(search.value||'');
    };
    const render=(q='')=>{
      q=q.trim().toLowerCase(); options.innerHTML='';
      Array.from(select.options).forEach(opt=>{
        if(q && !opt.textContent.toLowerCase().includes(q)) return;
        const row=document.createElement('label');
        row.className='multi-inline-option';
        row.innerHTML=`<input type="checkbox" ${opt.selected?'checked':''}><span>${opt.textContent}</span>`;
        row.querySelector('input').addEventListener('change',e=>{ opt.selected=e.target.checked; select.dispatchEvent(new Event('change',{bubbles:true})); update();});
        options.appendChild(row);
      });
    };
    trigger.addEventListener('click',()=>{ host.classList.toggle('open'); trigger.setAttribute('aria-expanded',host.classList.contains('open')?'true':'false'); });
    search.addEventListener('input',()=>render(search.value));
    document.addEventListener('click',e=>{ if(!host.contains(e.target)) { host.classList.remove('open'); trigger.setAttribute('aria-expanded','false'); } });
    if(cfg.help && !label.querySelector('.help-text')){ const p=document.createElement('div'); p.className='help-text'; p.textContent=cfg.help; host.after(p); }
    update();
  }

  function applyTerminology(){
    Object.entries(TERM_MAP).forEach(([value,label])=>{
      $$(`option[value="${value}"]`).forEach(opt=>opt.textContent=label);
    });
    const sport=$('#currentSport');
    if(sport){
      const map={none:'Aucun sport régulier',boxing:'Boxe',running:'Course / trail',gym:'Musculation / fitness',hyrox:'Hyrox / fonctionnel',team_sport:'Sport collectif',combat:'Sport de combat',health:'Remise en forme santé',other:'Autre'};
      Array.from(sport.options).forEach(o=>{ if(map[o.value]) o.textContent=map[o.value]; });
    }
    const activity=$('#activityLevel');
    if(activity){ const map={sedentary:'Sédentaire (peu de mouvement au quotidien)',moderate:'Modéré (marche / activité légère)',active:'Actif (bouge beaucoup)',very_active:'Très actif (travail physique / sport fréquent)'}; Array.from(activity.options).forEach(o=>{ if(map[o.value]) o.textContent=map[o.value];});}
    const life=$('#lifeContext');
    if(life){ const map={desk:'Travail assis',mixed:'Rythme mixte',physical:'Travail physique',shift:'Horaires décalés',parent:'Parent très occupé'}; Array.from(life.options).forEach(o=>{ if(map[o.value]) o.textContent=map[o.value];}); }
  }

  function uniformizeForms(){
    $$('label select:not([multiple]), label input, label textarea').forEach(decorateLabelFor);
    $$('select[multiple]').forEach(sel=>{ decorateLabelFor(sel); rebuildInlineMulti(sel); });
    ['#clientCode','#clientName','#clientEmail'].forEach(id=>{ const el=$(id); if(el) el.autocomplete='off'; });
    const summary=$('#coachSummary'); if(summary) summary.classList.add('summary-intel');
  }

  function fixSidebarBrand(){
    const h1=document.querySelector('.brand h1'); if(h1) h1.textContent='FAFATRAINING';
  }

  function explainStyle(style){
    return {
      emom:'Chaque minute démarre un exercice. Le temps restant dans la minute sert de récupération.',
      amrap:'On enchaîne le plus de tours propres possible dans le temps prévu, en gardant une technique correcte.',
      interval:'On alterne effort et récupération selon le ratio indiqué.',
      zone2:'Effort continu confortable, on peut encore parler sans être essoufflé.',
      strength:'On privilégie la qualité technique, la charge progressive et des repos plus longs.',
      mobility:'On cherche amplitude, contrôle et respiration, sans douleur.'
    }[style] || 'On suit les séries, répétitions ou temps indiqués, puis le repos prescrit.';
  }

  function prescriptionQuick(style, block, dur){
    const d=Number(dur||45);
    if(block==='warmup') return {series:'1 à 2 passages',reps:'30 à 40 sec / 6 à 10 reps',rest:'10 à 20 sec'};
    if(block==='cooldown') return {series:'1 passage',reps:'40 à 60 sec / respiration contrôlée',rest:'sans repos imposé'};
    if(style==='emom') return {series:d<=30?'8 à 10 minutes':'10 à 16 minutes',reps:'1 exercice chaque minute',rest:'reste de la minute'};
    if(style==='amrap') return {series:d<=30?'8 à 10 min':'10 à 16 min',reps:'tour continu',rest:'micro-pause si besoin'};
    if(style==='interval') return {series:d<=30?'6 à 8 blocs':'8 à 10 blocs',reps:'30 à 45 sec effort',rest:'15 à 30 sec'};
    if(style==='zone2') return {series:'1 bloc continu',reps:d<=30?'18 à 24 min':'25 à 40 min',rest:'pas de repos, effort régulier'};
    if(style==='strength') return {series:'3 à 5 séries',reps:'4 à 10 reps',rest:'75 à 150 sec'};
    if(style==='mobility' || style==='recovery') return {series:'2 à 3 passages',reps:'30 à 50 sec',rest:'10 à 20 sec'};
    return {series:'3 à 4 séries',reps:'8 à 15 reps / 30 à 45 sec',rest:'30 à 60 sec'};
  }

  function dedupeBase(list){
    const seen=new Set();
    return (list||[]).filter(ex=>{ const b=(window.baseName?window.baseName(ex):ex.name).toLowerCase(); if(seen.has(b)) return false; seen.add(b); return true; });
  }

  function betterGoalKeys(goal, env, title=''){
    const t=title.toLowerCase();
    if((env==='gym'||env==='crossfit_box'||env==='boxing_gym') && ['strength','muscle_gain','recomposition'].includes(goal)){
      if(/pouss|haut du corps 1|jour 1/.test(t)) return ['bench','press','chest','shoulders','triceps'];
      if(/tirage|haut du corps 2|jour 2/.test(t)) return ['row','pull','back','biceps','rear'];
      if(/squat|jambes dominant squat|jour 3/.test(t)) return ['squat','quads','glutes','split squat','leg press'];
      if(/charni|hinge|jour 4|jour 5/.test(t)) return ['hinge','hamstrings','glutes','rdl','hip thrust'];
      return ['bench','row','squat','hinge','press','pull'];
    }
    return window.goalKeys ? window.goalKeys(goal,env,title) : [goal];
  }

  function choosePack({level,env,equipment,style,goals,count,exclude}){
    const list=(window.chooseDiverseExercises?window.chooseDiverseExercises({level,env,equipment,style,goals,count:count*2,excludeBases:exclude||[]}):[])||[];
    return dedupeBase(list).slice(0,count);
  }

  function loadText(goal, env, idx, level){
    if(['strength','muscle_gain','recomposition'].includes(goal) && ['gym','crossfit_box','boxing_gym'].includes(env)){
      if(goal==='strength') return idx<2 ? 'Charge lourde contrôlée · RPE 7 à 9 · garder 1 à 3 répétitions en réserve' : 'Charge modérée à lourde · RPE 7 à 8';
      if(goal==='muscle_gain') return idx<2 ? 'Charge modérée à lourde · vise 1 à 2 répétitions en réserve' : 'Charge modérée · brûlure musculaire sans tricher';
      return 'Charge modérée progressive · exécution propre';
    }
    if(goal==='boxing') return 'Intensité technique à modérée · priorité précision / appuis';
    if(goal==='mobility' || goal==='health') return 'Sans charge ou charge légère · amplitude confortable';
    return 'Charge adaptée au niveau · garder une exécution propre';
  }

  function groupTitle(style){
    return {
      emom:'Bloc principal EMOM',
      amrap:'Bloc principal AMRAP',
      interval:'Bloc principal intervalles',
      zone2:'Bloc principal endurance fondamentale',
      strength:'Bloc principal force / musculation',
      mobility:'Bloc principal mobilité',
      recovery:'Bloc récupération active'
    }[style] || 'Bloc principal';
  }

  function frStyle(style){ return TERM_MAP[style] || (window.quickStyleLabel?window.quickStyleLabel(style):style); }

  window.buildProgramDays = function(form){
    const titles=(typeof window.titleSet==='function'?window.titleSet(form):null) || (window.buildDayTitles?window.buildDayTitles(form.freq,form.mainGoal,''):[`Jour 1`]);
    return titles.map((title,di)=>{
      const goals=betterGoalKeys(form.mainGoal, form.env, title);
      const isGym=['gym','crossfit_box','boxing_gym'].includes(form.env) && ['strength','muscle_gain','recomposition'].includes(form.mainGoal);
      const pack=choosePack({level:form.level,env:form.env,equipment:form.equipment,style:isGym?'strength':form.mainGoal,goals,count:isGym?4:5,exclude:[]});
      const items=pack.map((ex,idx)=>{
        const pres=window.formatPrescription?window.formatPrescription(form.mainGoal,title,idx):{series:'3 séries',reps:'8 à 12 reps',rest:'45 sec'};
        if(isGym){
          pres.series = idx<2 ? '4 séries' : '3 séries';
          pres.reps = form.mainGoal==='strength' ? (idx<2?'4 à 6 reps':'6 à 8 reps') : (idx<2?'6 à 10 reps':'10 à 15 reps');
          pres.rest = idx<2 ? '90 à 150 sec' : '60 à 90 sec';
        }
        return {
          ...ex,
          muscles: ex.muscles || goals.slice(0,2).join(', '),
          category: ex.category || 'exercice',
          prescription: pres,
          load: loadText(form.mainGoal, form.env, idx, form.level),
          cue: ex.cue || 'Exécution propre, amplitude maîtrisée et respiration régulière.',
          substitute: ex.easy || ex.substitute || 'Choisir une variante plus stable ou réduire la charge.',
          progression: ex.hard || ex.progression || 'Augmenter légèrement la charge, l’amplitude ou la difficulté technique.',
          coachNote: ex.coachNote || 'Valider la qualité du mouvement avant d’augmenter le niveau.'
        };
      });
      return {title,patternSummary:(window.GOAL_EXPLAIN&&window.GOAL_EXPLAIN[form.mainGoal])||'répartition cohérente selon objectif',style:isGym?'strength':form.mainGoal,items};
    });
  };

  window.renderProgram = function(p, coach=true){
    const week=(window.buildWeekSchedule?window.buildWeekSchedule(p.freq):[]).map((w,i)=>({day:w.day||['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'][i],note:w.note||'Repos actif',train:!!w.train}));
    const head=`<div class="panel panel-hero clean-program-head"><h3>${esc(p.name)} · ${esc(p.code)}</h3><div class="meta"><span class="program-code">Code ${esc(p.code)}</span>${coach?'<span class="badge">Accès adhérent prêt</span>':''}</div><div class="meta"><span class="badge">${esc(window.labelForGoal?window.labelForGoal(p.mainGoal):p.mainGoal)}</span><span class="badge">${esc(window.labelForEnv?window.labelForEnv(p.env):p.env)}</span><span class="badge">${esc(window.labelForLevel?window.labelForLevel(p.level):p.level)}</span><span class="badge">${esc(p.freq)} séance(s) / semaine</span><span class="badge">${esc(p.duration)} min</span></div><div class="summary-grid summary-grid-4 compact-kpis"><div class="summary-card"><strong>${esc(agePublicLabel(p.age))}</strong><div class="small-muted">public</div></div><div class="summary-card"><strong>${p.bmi?esc(p.bmi.value):'-'}</strong><div class="small-muted">IMC officiel</div></div><div class="summary-card"><strong>${esc(p.cycleWeeks||8)} sem.</strong><div class="small-muted">cycle</div></div><div class="summary-card"><strong>${esc((p.days||[]).length)}</strong><div class="small-muted">séances</div></div></div></div>`;
    const sched=`<article class="athlete-week"><h3>Semaine type</h3><div class="week-days compact">${week.map(w=>`<div class="daypill ${w.train?'train':'rest'}"><strong>${esc(w.day)}</strong><small>${esc(w.note)}</small></div>`).join('')}</div></article>`;
    ...
    const days=(p.days||[]).map(day=>`<article class="panel program-day-block"><div class="program-day-header"><div><h3>${esc(day.title)}</h3><p class="small-muted">Répartition : ${esc(day.patternSummary)}</p></div><span class="badge">${esc(frStyle(day.style||p.mainGoal||''))}</span></div><div class="program-ex-grid">${(day.items||[]).map(ex=>`<div class="program-ex-card"><h4>${esc(ex.name)}</h4><div class="small-muted">${esc(ex.category||'')} · ${esc(ex.muscles||'')}</div><div class="meta-line"><span>${esc(ex.prescription.series)}</span><span>${esc(ex.prescription.reps)}</span><span>Repos ${esc(ex.prescription.rest)}</span><span>${esc(ex.load||'Charge adaptée')}</span></div><div class="session-line"><strong>Consigne :</strong> ${esc(ex.cue||'')}</div><div class="session-line"><strong>Version plus facile :</strong> ${esc(ex.substitute||'')}</div><div class="session-line"><strong>Version plus difficile :</strong> ${esc(ex.progression||'')}</div><div class="session-line"><strong>Note coach :</strong> ${esc(ex.coachNote||'')}</div></div>`).join('')}</div></article>`).join('');
    return head+sched+actions+`<div class="stack">${days}</div>`;
  };

  window.quickBuild = function(){
    const style=$('#quickStyle')?.value || 'circuit';
    const dur=Number($('#quickDuration')?.value || 45);
    const env=$('#quickEnv')?.value || 'home';
    const audience=$('#quickPublic')?.value || 'adults';
    const realStyle={amrap:'conditioning',emom:'conditioning',interval:'conditioning',zone2:'trail'}[style] || style;
    const level=audience==='athletes'?'advanced':audience==='seniors'?'beginner':'intermediate';
    const equipment=(window.PRESETS&&window.PRESETS[env])||[];
    const warmGoals=realStyle==='mobility'?['mobility','rotation','stability']:realStyle==='strength'?['mobility','activation','core']:['mobility','cardio','core'];
    const mainGoals=betterGoalKeys(realStyle,env,'bloc principal');
    const finishGoals=realStyle==='mobility'||realStyle==='recovery'?['breathing','mobility','stability']:(realStyle==='strength'?['core','mobility','breathing']:['core','mobility','cardio']);
    const warm=choosePack({level,env,equipment,style:realStyle,goals:warmGoals,count:2,exclude:[]});
    const main=choosePack({level,env,equipment,style:realStyle,goals:mainGoals,count:4,exclude:warm.map(x=>x.name)});
    const fin=choosePack({level,env,equipment,style:realStyle,goals:finishGoals,count:2,exclude:warm.concat(main).map(x=>x.name)});
    const sections=[['Échauffement','mise en route progressive',warm,'warmup'],[groupTitle(style),explainStyle(style),main,'main'],['Retour au calme','retour au calme / respiration / mobilité',fin,'cooldown']];
    const html=`<div class="quick-layout"><div class="panel panel-hero">${sections.map(([title,sub,list,key])=>{const pres=prescriptionQuick(style,key,dur);return `<div class="quick-block"><h4>${title}</h4><div class="section-subtitle">${sub}</div><div class="quick-list">${list.map(ex=>`<div class="session-item"><strong>${esc(ex.name)}</strong><div class="small-muted">${esc(ex.category||'')} · ${esc(ex.muscles||'')}</div><div class="quick-prescription"><span>${esc(pres.series)}</span><span>${esc(pres.reps)}</span><span>Repos ${esc(pres.rest)}</span><span>${esc(loadText(realStyle,env,0,level))}</span></div><div class="session-line"><strong>Consigne :</strong> ${esc(ex.cue||'Exécution propre et respiration maîtrisée.')}</div><div class="session-line"><strong>Version plus facile :</strong> ${esc(ex.easy||ex.substitute||'Réduire l’amplitude ou choisir une variante plus stable.')}</div><div class="session-line"><strong>Version plus difficile :</strong> ${esc(ex.hard||ex.progression||'Ajouter un peu d’amplitude, de vitesse ou de charge.')}</div></div>`).join('')}</div></div>`;}).join('')}</div><div class="quick-rail"><div class="panel"><h3>Logique de séance</h3><div class="small-muted"><strong>Style :</strong> ${esc(frStyle(style))}</div><div class="small-muted"><strong>Temps de repos :</strong> ${esc(explainStyle(style))}</div><div class="small-muted"><strong>Lecture pratique :</strong> on termine les séries ou le temps prévu pour un exercice, puis on prend le repos indiqué avant de passer au suivant, sauf si le format impose un enchaînement continu.</div><div class="small-muted"><strong>Charge :</strong> ${esc(loadText(realStyle,env,0,level))}</div></div><div class="panel"><h3>Glossaire</h3><div class="nutrition-list"><div><strong>HIIT :</strong> intervalles haute intensité</div><div><strong>EMOM :</strong> chaque minute sur la minute</div><div><strong>AMRAP :</strong> maximum de tours</div><div><strong>Zone 2 :</strong> endurance fondamentale</div></div></div></div></div>`;
    $('#quickOutput').innerHTML=html;
  };

  const oldExport=window.exportPdf;
  window.exportPdf=function(){ return oldExport ? oldExport() : null; };

  function repairSmallIssues(){
    const coachDesc=document.querySelector('#coach .section-head p'); if(coachDesc) coachDesc.textContent='Workflow en 3 étapes : Profil, Objectifs, Matériel. Champs homogènes, utiles et mieux expliqués.';
    $$('.step').forEach((b,i)=>b.textContent=['1. Profil','2. Objectifs','3. Matériel'][i]||b.textContent);
    const summaryLabels={ 'Client :':'Client :', 'Objectif :':'Objectif :', 'Contexte :':'Contexte :', 'Fréquence conseillée :':'Fréquence conseillée :', 'Durée conseillée :':'Durée conseillée :', 'IMC :':'IMC :', 'Tour de taille :':'Tour de taille :', 'Modules :':'Modules :', 'Statut business :':'Statut business :' };
  }
  ...
  function bootV301(){
    ...
    fixSidebarBrand();
    repairSmallIssues();
    const quickStyle=$('#quickStyle'); if(quickStyle){ quickStyle.addEventListener('change',()=>{ if($('#quickOutput')?.innerHTML.trim()) window.quickBuild(); }); }
    ['#quickDuration','#quickEnv','#quickPublic'].forEach(id=>$(id)?.addEventListener('change',()=>{ if($('#quickOutput')?.innerHTML.trim()) window.quickBuild(); }));
  }

  document.addEventListener('DOMContentLoaded',()=>setTimeout(bootV301,350));
})();
